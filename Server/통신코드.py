...
#4. 클라] 다른 클라이언트와 연결이 될 때까지 대기하던 중 취소 버튼을 누르면 연결 취소 코드 전송 (Code: "Cancel")

#5  서버] 이름 전송 받고 클라이언트가 두 개 접속인지 확인, 두 개 접속했을 경우 두 클라이언트에 게임 시작 코드 전송 (Code: "Stage 0 to 1")
#5-1. 서버] 두 개 이상 접속일 경우 연결 해제 코드 전송 (Code: "Break") 
#5-2. 클라] Break 코드 시 연결 해제

#6  서버, 클라] 3초 대기 후 다음 스테이지로 이동
...
#2. 클라] 10초 이내에 가위바위보 값 입력 (Code: "Stage1Input: " + "ROCK" or "SCISSORS" or "PAPER")
#2-1.클라] 10초 이내로 결정하지 못할 시 코드 전송 (Code: "Stage1Input: " + "Undecided")
...
#3. 서버] 입력받은 두 가위바위보 값 서버 내에서 비교한 후 각자의 스레드 1, 2에 이겼다/아니다/무승부 코드 전송해준다. (Code: "Stage1: " + "Win" or "Lose" or "Draw")
...
#2. 서버] 현재 누구 턴인지 각 스레드에서 전송 (Code: "Turn" or "Not Turn")
#2-1 클라] 서버의 메시지에 따라 클라이언트에 표시

#3. 서버] 이용자에 묵찌빠 값 입력하라고 보낸다. (Code: "Receiving Stage 2")

#4. 클라] 10초 이내에 묵찌빠 값 입력 (Code: "Stage2Input: " + "ROCK" or "SCISSORS" or "PAPER")
#4-1.클라] 10초 이내로 결정하지 못할 시 코드 전송 (Code: "Stage2Input: " + "Undecided")
...
#5. 서버] 입력받은 묵찌빠 값 비교하고 같은 입력이 아니라면 현재 턴이 누구인지 판단하여 저장하고 각 스레드에서 전송 (Code: "Turn" or "Not Turn")
#5-1. 서버, 클라] #2-1부터 반복

#6. 서버] 만일 같은 입력이 등장했다면 턴을 가진 사람의 점수를 +1, 승리 코드 전송 (Code: "Stage2: " + "Win" or "Lose")
